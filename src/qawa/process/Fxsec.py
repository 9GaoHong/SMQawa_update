...
# def build_vbs_jets(jets, leptons=None, pt_threshold=30.0, eta_max=4.7, dr_min=0.01):
#     """
#     Apply a common VBS jet selection for both reco and gen collections.
#     Returns the cleaned/sorted jets, the leading/subleading/third entries,
#     and the boolean mask applied to the original collection.
#     """
#     mask = (jets.pt > pt_threshold) & (np.abs(jets.eta) < eta_max)
#     if leptons is not None:
#         overlap = ak.any(jets.metric_table(leptons) <= 0.4, axis=2)
#         mask = mask & (~overlap)

#     selected = jets[mask]
#     order = ak.argsort(selected.pt, axis=1, ascending=False)
#     selected = selected[order]

#     lead = ak.firsts(selected)
#     subl = ak.firsts(selected[lead.delta_r(selected) > dr_min])
#     third = ak.firsts(
#         selected[
#             (lead.delta_r(selected) > dr_min) &
#             (subl.delta_r(selected) > dr_min)
#         ]
#     )

#     return selected, lead, subl, third, mask
...
class zzinc_processor(processor.ProcessorABC):
    ...
    def __init__(self, era: str ='2018', isDY=False, dd='SR',dump_gnn_array=False, ewk_process_name=None, run_period: str = ''): 
        ...
        self.build_histos = lambda: {
            'dijetmass_fv_vs_reco': hist.Hist(
                hist.axis.StrCategory([], name="channel",    growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(100, 0, 2000,     name="dijet_mass_fv", label=r"$m_{jj}$ (GeV, truth)"),
                hist.axis.Regular(100, 0, 2000,  name="dijet_mass",    label=r"$m_{jj}$ (GeV, reco)"),
                hist.storage.Weight()
            ),
            'dijet_deta_fv_vs_reco': hist.Hist(
                hist.axis.StrCategory([], name="channel",    growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(40, 0, 8,     name="dijet_deta_fv", label=r"$Delta\eta_{jj}$ (truth)"),
                hist.axis.Regular(40, 0, 8,  name="dijet_deta",    label=r"$Delta\eta_{jj}$ (reco)"),
                hist.storage.Weight()
            ),
            ...
            'met_pt': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True), 
                hist.axis.Regular(100, 0, 1000, name="met_pt", label=r"$p_{T}^{miss}$ (GeV)"),
                hist.storage.Weight()
            ),
            'met_pt_fv': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True), 
                hist.axis.Regular(100, 0, 1000, name="met_pt_fv", label=r"$p_{T}^{miss}$ (GeV)"),
                hist.storage.Weight()
            ),
            'dijet_mass_fv': hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(50, 0, 2000, name="dijet_mass_fv", label=r"$m_{jj}$ (GeV, truth)"),
                hist.storage.Weight()
            ),
            'dijet_deta_fv': hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(20, 0, 8, name="dijet_deta_fv", label=r"$Delta\eta_{jj}$ (truth)"),
                hist.storage.Weight()
            ),
            'dijet_mass_fv_noclean': hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(50, 0, 2000, name="dijet_mass_fv_noclean", label=r"$m_{jj}$ (GeV, truth)"),
                hist.storage.Weight()
            ),
            'dijet_deta_fv_noclean': hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(20, 0, 8, name="dijet_deta_fv_noclean", label=r"$Delta\eta_{jj}$ (truth)"),
                hist.storage.Weight()
            ),
            'met_sigma': hist.Hist(
                hist.axis.StrCategory([], name="channel"   , growth=True),
            ...
            "lead_jet_pt_fv": hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(50, 30, 530, name="lead_jet_pt_fv", label=r"$p_T^{j_1}$ (GeV, truth)"),
                hist.storage.Weight()
            ),
            "trail_jet_pt_fv": hist.Hist(
                hist.axis.StrCategory([], name="channel", growth=True),
                hist.axis.StrCategory([], name="systematic", growth=True),
                hist.axis.Regular(50, 30, 530, name="trail_jet_pt_fv", label=r"$p_T^{j_2}$ (GeV, truth)"),
                hist.storage.Weight()
            ),
    ...
    def process_shift(self, event, shift_name:str=''):
        ...
        overlap = ak.any(jets.metric_table(tight_lep_fv) <= 0.4, axis=2)
         
        good_jets_fv = jets[jet_mask & (~overlap)]
        ...
        #noclean jets
        good_jets_fv_noclean = jets[jet_mask]
        ngood_jets_fv_noclean = ak.num(good_jets_fv_noclean)
        sorted_indices = np.argsort(-good_jets_fv_noclean.pt)
        good_jets_fv_noclean = good_jets_fv_noclean[sorted_indices]
        
        lead_jet_fv = ak.firsts(good_jets_fv_noclean)
        subl_jet_fv = ak.firsts(good_jets_fv_noclean[lead_jet_fv.delta_r(good_jets_fv_noclean)>0.01])
        third_jet_fv = ak.firsts(good_jets_fv_noclean[(lead_jet_fv.delta_r(good_jets_fv_noclean)>0.01) & (subl_jet_fv.delta_r(good_jets_fv_noclean)>0.01)])
        dijet_mass_fv_noclean = (lead_jet_fv + subl_jet_fv).mass
        dijet_deta_fv_noclean = np.abs(lead_jet_fv.eta - subl_jet_fv.eta)
        ...
        selection.add('low_met_pt_fv', ak.fill_none(p4_met_fv.pt > 120, False))
        selection.add('low_met_pt', ak.fill_none((reco_met_pt > 120), False))
        ...
        selection.add('dilep_pt_fv', ak.fill_none(dilep_pt_fv>60, False))
        selection.add(
            'dilep_pt',
            ak.where(
        ...
        selection.add('2njets_fv' , ngood_jets_fv  >= 2 )
        ...
        event['ngood_jets_fv_noclean'  ] = ak.fill_none(ngood_jets_fv_noclean,-99)
        ...
        event['dijet_mass_fv'] = ak.fill_none(dijet_mass_fv,-99)
        event['dijet_deta_fv'] = ak.fill_none(dijet_deta_fv,-99)
        event['dijet_mass_fv_noclean'] = ak.fill_none(dijet_mass_fv_noclean,-99)
        event['dijet_deta_fv_noclean'] = ak.fill_none(dijet_deta_fv_noclean,-99)
        ...
        truth_FV = [
            'require-ossf-fv','dilep_m_fv','dilep_pt_fv','2njets_fv','low_met_pt_fv',
            'dijet_deta_fv','dijet_mass_400_fv',
        ]
        ...
        pass_reco  = selection.require(**bits_to_kwargs(reco_SR))
        pass_truth = selection.require(**bits_to_kwargs(truth_FV))

        # 分类
        channels = {
            "both-SR":       pass_reco & pass_truth,
            "truth-only": ~pass_reco & pass_truth,
            "reco-only": pass_reco & ~pass_truth,
            "none":       ~pass_reco & ~pass_truth,
            "reco":       pass_reco,
            "truth":      pass_truth,
        }
        ...
        for ch in channels:
            for sys in systematics:
                for var in (
                    ...
                    'gnn_flat',
                    'ngood_jets_fv',
                    'ngood_jets_fv_noclean',
                    'dijet_deta_fv_noclean',
                    'dijet_mass_fv_noclean',
                    'dijet_mass_fv',
                    'dijet_deta_fv',
                ):
                    _histogram_filler(ch, sys, var)
                _hist2d_filler(ch, None, 'dijet_deta_fv', 'dijet_deta',hist_key='dijet_deta_fv_vs_reco')
                _hist2d_filler(ch, None, 'dijet_mass_fv', 'dijet_mass',hist_key='dijetmass_fv_vs_reco')
...
#            #    "/tmp/hgao/ZZTo2Mu2Nu-MC_0.root",
#            #    "/tmp/hgao/ZZTo2Mu2Nu-MC_1.root",
# ],
#           'metadata':{
#               'era': "2018",
#               'is_data': False
...
# #    maxchunks=3
# )
# #with gzip.open(f"FV_hist.pkl.gz", "wb") as fout:
# #    pickle.dump(out_btag1, fout)
# #
# print('both',out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'both', 'systematic': 'nominal'}].values().sum())
# print('reco-only',out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'reco-only', 'systematic': 'nominal'}].values().sum())
# # print('truth-only',out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'truth-only', 'systematic': 'nominal'}].values().sum())
# print('reco',out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'reco', 'systematic': 'nominal'}].values().sum())
# print('truth',out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'truth', 'systematic': 'nominal'}].values().sum())
#
#SR = out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'reco', 'systematic': 'nominal'}].values().sum()
#both = out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'both', 'systematic': 'nominal'}].values().sum()
#print("ratio  ",both/SR)
#print(out_btag1['ZZTo2E2Nu_TuneCP5_DipoleRecoil_13TeV_powheg_pythia8']['met_pt'][{'channel': 'truth', 'systematic': 'nominal'}].values().sum())
#
